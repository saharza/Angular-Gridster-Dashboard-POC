				...
				function dragStop(event) {
					...
					scope.$apply(function() {
						if (gridster.draggable && gridster.draggable.stop) {
                            // TODO: validate if it's working => draggable.stop: function(event, $element, widget, newPosition) {}
				...
				this.enable = function() {
					...
					$el.ready(function() {
						...
						for (var h = 0, hl = $dragHandles.length; h < hl; ++h) {
                            //// CUSTOMIZATION - adding draggable for each widget
	...
	.factory('GridsterResizable', ['GridsterTouch', function(GridsterTouch) {
		function GridsterResizable($el, scope, gridster, item, itemOptions) {
				...
				function resizeStop(e) {
					...
					scope.$apply(function() {
						if (gridster.resizable && gridster.resizable.stop) {
                            // TODO: validate if it's working => resizable.stop: function(event, $element, widget, newSize) {}
                            gridster.resizable.stop(e, $el, itemOptions, newSize);
			...
			for (var c = 0, l = handlesOpts.length; c < l; c++) {
				handles.push(new ResizeHandle(handlesOpts[c])); // OLD CODE
                //// CUSTOMIZATION - adding resizeable for each widget
			...
			return {
				...
				link: function(scope, $el, attrs, controllers) {
					...
					function sizeChanged() {
                        var maxLeft = gridster.curWidth - 1,
                            curColWidth = gridster.curColWidth,
                            elmX = parseInt($el.css('left'), 10),
                            elmW = $el[0].offsetWidth,
                            changedX, changedY;

                        ////////////////////////////////////////////////////////////////////////////////
                        // TODO: if element overlapping the dashboard border the shift it to the left...
                        /// by using the => item.setPosition(item.row, item.col) ||
                        /// just set a new row/col (item.row / item.col) => this should trigger the 'positionChanged()' method
                        /// after that try to set the sizeX...
                        ////////////////////////////////////////////////////////////////////////////////
                        //if(item.sizeX > item.oldSizeX){
                        //    if (elmX + elmW + (curColWidth * (item.sizeX - item.oldSizeX)) <= maxLeft) {
                        //        changedX = item.setSizeX(item.sizeX, true);
                        //    }else{
                        //        changedX = item.setSizeX(item.oldSizeX, true);
                        //    }
                        //}else{
                        //    changedX = item.setSizeX(item.sizeX, true);
                        //}
                        ...
                        changedY = item.setSizeY(item.sizeY, true);
